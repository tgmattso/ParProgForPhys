...
PURPOSE: This MPI program measures communication times for
         ...
         statistics are compiled.  This is the MPI version
         of the ping_pong benchmarks published around 1990,
         so the style of programming is quite old.
...
HISTORY: Written by Tim Mattson, April 1999.  This is based on the
         p4 ping_pong program I wrote long ago at Yale (~1990).
...
long l_random();
double wt_overhead(long); /* MPI_Wtime correction factor in Seconds */
void time_stats(double *, int, int);  

...
FUNCTIONS CALLED:
         ...
         functions are used in this program:
          ...
          trans_snd_rcv()   Do the transpose with snd's and rcv's
          trans_sendrcv()   Do the transpose with MPI_Sendrecv()
          trans_isnd_ircv() Do the transpose with isnd's and ircv's
...
int main(int argc, char ** argv)
{
   ...
   double errsq_snd_rcv;   /* squared error, Blocking Send, Recv */
   double errsq_isnd_ircv; /* squared error, immediate mode */
   double errsq_sendrcv;   /* squared error, Sendrecv */

   double t0, trans_time;

   double min_time_snd_rcv,   max_time_snd_rcv, min_max_time_snd_rcv;
   int    min_iter_snd_rcv,   max_iter_snd_rcv, min_max_iter_snd_rcv;

   double min_time_isnd_ircv, max_time_isnd_ircv, min_max_time_isnd_ircv;
   int    min_iter_isnd_ircv, max_iter_isnd_ircv, min_max_iter_isnd_ircv;
   ...
   void   trans_snd_rcv   (double*, double*, int, double*, int, int);
   void   trans_isnd_ircv (double*, double*, int, double*, int, int);
...
/*********************************************************************
** Transpose the full column matrix.  Do this multiple times and
** keep trask of the minimum and maximum time
**
** Do the Deadlock senstive, SND_RCV transpose..
*********************************************************************/
  if (Block_order <= MAX_SND_RCV){
     if(my_ID == 0){
        min_time_snd_rcv     = 1/TOL;
        max_time_snd_rcv     = TOL;
        min_max_time_snd_rcv = 1/TOL;
        min_iter_snd_rcv     = -1;
        max_iter_snd_rcv     = -1;
        min_max_iter_snd_rcv = -1;
     }   
     ...
     for (iters = 0; iters<iterations; iters++){
        ...
        trans_snd_rcv(buff,  trans, Block_order,
                work,  my_ID, Num_procs);

        trans_time = MPI_Wtime() - t0;

#ifdef DEBUG
     printf("\n Node %d has finished with snd_rcv transpose \n",my_ID);
     fflush(stdout);
#endif

    
        analyze_results (my_ID, trans_time, iters,
            &min_time_snd_rcv, &max_time_snd_rcv, &min_max_time_snd_rcv, 
            &min_iter_snd_rcv, &max_iter_snd_rcv, &min_max_iter_snd_rcv );

        errsq_snd_rcv +=  test_results (my_ID, Block_order, Global_order, 
                                     trans);
...
/*********************************************************************
** Do the immediate mode, ISND_IRCV transpose..
*********************************************************************/
  if(my_ID == 0){
     min_time_isnd_ircv     = 1/TOL;
     max_time_isnd_ircv     = TOL;
     min_max_time_isnd_ircv = 1/TOL;
     min_iter_isnd_ircv     = -1;
     max_iter_isnd_ircv     = -1;
     min_max_iter_isnd_ircv = -1;
  }   
  fill_garbage(my_ID, Global_order, Block_order, trans);
  errsq_isnd_ircv = 0.0;
  ...
  for (iters = 0; iters<iterations; iters++){
     ...
     trans_isnd_ircv(buff,  trans, Block_order,
                work,  my_ID, Num_procs);
     ...
     analyze_results (my_ID, trans_time, iters,
         &min_time_isnd_ircv, &max_time_isnd_ircv, &min_max_time_isnd_ircv, 
         &min_iter_isnd_ircv, &max_iter_isnd_ircv, &min_max_iter_isnd_ircv );

     errsq_isnd_ircv =  test_results (my_ID, Block_order, 
		                     Global_order, trans);
...
*********************************************************************/
   if(my_ID == 0){
     if (Block_order <= MAX_SND_RCV){
        printf("\n Send-Recv Transpose Test Results ");
        output_timings ("Send-Recv",Num_procs, Block_order,
         min_time_snd_rcv,   max_time_snd_rcv, min_max_time_snd_rcv, 
         min_iter_snd_rcv,   max_iter_snd_rcv, min_max_iter_snd_rcv );
     }
     ...
     printf("\n ISend-IRecv Transpose Test Results ");
     output_timings ("ISnd-IRcv",Num_procs, Block_order,
         min_time_isnd_ircv, max_time_isnd_ircv, min_max_time_isnd_ircv, 
         min_iter_isnd_ircv, max_iter_isnd_ircv, min_max_iter_isnd_ircv);

     printf("\n squared errors: %f %f %f \n",
              errsq_snd_rcv, errsq_sendrcv, errsq_isnd_ircv);
   }

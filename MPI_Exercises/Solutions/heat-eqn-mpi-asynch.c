...
int main(int argc, char *argv[]) {
    ...
    int numPoints;
    MPI_Status status; 
    MPI_Request reqRecvL, reqRecvR, reqSendL, reqSendR;
    ...
    for (int k = 0; k < NSTEPS; ++k) {
        ...
        if (myID != 0) {
            MPI_Irecv(&uk[0], 1, MPI_DOUBLE, leftNbr, 0, MPI_COMM_WORLD, 
                    &reqRecvL);
            MPI_Isend(&uk[1], 1, MPI_DOUBLE, leftNbr, 0, MPI_COMM_WORLD,
                    &reqSendL);
        }
        if (myID != numProcs-1) {
            MPI_Irecv(&uk[numPoints+1],1, MPI_DOUBLE, rightNbr, 0, MPI_COMM_WORLD,
                    &reqRecvR);
            MPI_Isend(&uk[numPoints], 1, MPI_DOUBLE, rightNbr, 0, MPI_COMM_WORLD,
                    &reqSendR);
        }

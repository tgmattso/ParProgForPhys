...
*********************************************************************/
  ...
  ***********************/
  ...
  ring_split(x,incoming,buff_count,num_procs,num_shifts,my_ID);
  MPI_Barrier(MPI_COMM_WORLD);
...
#ifdef DEBUG
  printf("\n Node %d has finished with ring_split \n",my_ID);
  fflush(stdout);
#endif
  ...
  ***********************/
  ...
  ring_sndrcv(x,incoming,buff_count,num_procs,num_shifts,my_ID);
  MPI_Barrier(MPI_COMM_WORLD);
...
#ifdef DEBUG
  printf("\n Node %d has finished with ring_sndrcv \n",my_ID);
  fflush(stdout);
#endif
   ...
   */
   ...
   if(my_ID == 0){
     printf("\n\n SendRecv Ring Test Results ");
     out_results(min_time, max_time, num_procs, buff_count, 
		                                          num_shifts);
   }

  /***************************
  ** Persistent Comm Ring Test
  ***************************/

  ring_persistent(x,incoming,buff_count,num_procs,num_shifts,my_ID);
  MPI_Barrier(MPI_COMM_WORLD);
...
#ifdef DEBUG
  printf("\n Node %d has finished with ring_persistent \n",my_ID);
  fflush(stdout);
#endif
   ...
   */
   ...
   if(my_ID == 0){
     printf("\n\n Persistent Comm Ring Test Results ");
     out_results(min_time, max_time, num_procs, buff_count, 
		                                          num_shifts);
   }
